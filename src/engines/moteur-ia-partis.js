...
import { getToutesLoisPartis } from '../data/lois/lois-partis.js'

// Catalogue unifié : lois génériques + lois de partis (sans doublons)
function getCatalogueComplet() {
  const base = Object.values(CATALOGUE_LOIS).flat()
  const idsBase = new Set(base.map(l => l.id))
  const partis = getToutesLoisPartis().filter(l => !idsBase.has(l.id))
  return [...base, ...partis]
}
...
export function soumettreLoiAuVote(loiId, etatJeu, hemicycle = HEMICYCLE_DEFAULT, bonusVote = 0) {
  const toutes = getCatalogueComplet()
...
/** Récupère une loi par son id dans le catalogue complet */
export function getLoi(loiId) {
  return getCatalogueComplet().find(l => l.id === loiId) ?? null
}

/** Toutes les lois (catalogue + partis) */
export function getToutesLois() {
  return getCatalogueComplet()
}
