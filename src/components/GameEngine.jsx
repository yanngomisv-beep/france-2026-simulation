...
function getEtatBase(partiId) {
  ...
  return {
    ...
    hemicycle: { ...HEMICYCLE_INITIAL }, lois_votees: [], scandales_actifs: [],
...
export default function GameEngine({ partiJoueur, children }) {
  ...
  const passerTour = useCallback(() => {
    ...
    setEtatJeu(prev => {
      let etat = { ...prev }
      let geo  = { ...etatGeo }
      try { const r = tourIA(etat); if (r?.etat) etat = { ...etat, ...r.etat }; if (r?.evenements?.length) evs.push(...r.evenements) } catch (e) { console.warn('tourIA:', e.message) }
      try {
        const etatEnergie = { prix_baril_dollars: etat.prix_baril_dollars ?? 80, prix_gaz_mwh: etat.prix_gaz_mwh ?? 38, prix_electricite_marche_mwh: etat.prix_electricite_marche_mwh ?? 72, mer_rouge_fermee: etat.mer_rouge_fermee ?? false, tensions_iran: etat.tensions_iran ?? false, edf_rentable: etat.edf_rentable ?? true, edf_dette_milliards: etat.edf_dette_milliards ?? 54, avancement_epr2_pct: etat.avancement_epr2_pct ?? 12, recettes_vnu_milliards: etat.recettes_vnu_milliards ?? 0 }
        ...
        if (r?.prix_electricite != null) { etat.prix_electricite = r.prix_electricite; etat.prix_electricite_marche_mwh = r.prix_electricite }
        if (r?.evenements_declenches?.length) evs.push(...r.evenements_declenches.map(e => ({ titre: e.titre, emoji: e.emoji ?? '⚡' })))
      ...
      try {
        ...
        if (r?.evenements_declenches?.length) evs.push(...r.evenements_declenches.map(e => ({ titre: e.titre, emoji: e.theatre_emoji ?? '🌍' })))
  ...
  const voterLoi = useCallback((loiId, bonusVote = 0) => {
    setEtatJeu(prev => {
      try { const res = soumettreLoiAuVote(loiId, prev, prev.hemicycle, bonusVote); if (res?.etat) return res.etat; return prev }
  ...
  const resoudreCrise = useCallback((crise) => {
    setEtatJeu(prev => { let e = { ...prev }; Object.entries(crise.impacts ?? {}).forEach(([k, v]) => { e[k] = Math.max(-9999, Math.min(9999, (e[k] ?? 0) + v)) }); return e })
  ...
  const gameProps = {
    etatJeu, etatGeo, curseurs, passerTour, voterLoi, appliquerLoiAdoptee, resoudreCrise,
