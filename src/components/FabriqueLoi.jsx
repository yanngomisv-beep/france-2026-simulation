...
const PROFILS = [
  ...
  { id: 'commercant',  label: 'Commerçant',  emoji: '🛒' },
...
Le joueur est Président de la République française en 2026. Contexte économique :
...
function CarteProfilImpact({ profil, data }) {
  ...
  return (
    <div className={`rounded-lg border p-3 flex flex-col gap-1 ${couleurs[data.impact] ?? couleurs.neutre}`}>
...
function CartePartiReaction({ nom, data }) {
  ...
  return (
    <div className={`rounded-lg border px-3 py-2 flex flex-col gap-1 ${couleurs[data.position] ?? couleurs.abstention}`}>
...
export default function FabriqueLoi({ etatJeu, appliquerLoiAdoptee, voterLoi }) {
    ...
    try {
      const response = await fetch('/api/claude', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          messages: [{ role: 'user', content: construirePrompt(intention, etatJeu) }],
        }),
      })
  ...
  function adopterLoi() {
    ...
    // Applique les impacts directement via appliquerLoiAdoptee (GameEngine)
    // en passant un ID unique + les impacts de la loi générée par l'IA
    const loiId = `LOI_FABRIQUE_${Date.now()}`
    if (appliquerLoiAdoptee) {
      appliquerLoiAdoptee(loiId, loiGeneree.impacts)
    }
    setAdoptee(true)
  }
    ...
    <div className="max-w-6xl mx-auto flex flex-col gap-6 pb-12">
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-6 flex flex-col gap-4">
        ...
        <textarea
          ...
          placeholder="Exemple : Je veux obliger les cliniques privées à participer aux gardes de nuit, ou augmenter le SMIC à 1700€..."
        ...
        >
          {loading ? '⚙️ Le Conseil Juridique analyse...' : '🏛️ Soumettre au Conseil Juridique'}
        </button>
      </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-slate-800 rounded-xl border border-blue-800 p-6">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                <p className="text-xs text-blue-400 uppercase tracking-wide mb-1">Projet de loi généré</p>
      ...
      {adoptee && loiGeneree && (
        <div className="bg-green-900/30 border border-green-700 rounded-xl p-8 text-center flex flex-col gap-4">
          ...
          <h3 className="text-xl font-bold text-green-300">"{loiGeneree.titre_officiel}" promulguée</h3>
          <p className="text-sm text-slate-400">
            La loi entre en vigueur. Les impacts économiques et sociaux sont appliqués immédiatement.
          </p>
          <button
